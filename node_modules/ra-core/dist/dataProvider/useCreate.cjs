...
const useMutationWithMutationMode_1 = require("./useMutationWithMutationMode.cjs");
/**
 ...
 *                    \-- data is Product
 */
const useCreate = (resource, params = {}, options = {}) => {
    const dataProvider = (0, useDataProvider_1.useDataProvider)();
    const queryClient = (0, react_query_1.useQueryClient)();
    const { mutationMode = 'pessimistic', getMutateWithMiddlewares, onSettled, ...mutationOptions } = options;
    const dataProviderCreate = (0, util_1.useEvent)((resource, params) => dataProvider.create(resource, params));
    const [mutate, mutationResult] = (0, useMutationWithMutationMode_1.useMutationWithMutationMode)({ resource, ...params }, {
        ...mutationOptions,
        ...
        updateCache: ({ resource, ...params }, { mutationMode }, result) => {
            const id = mutationMode === 'pessimistic'
                ? result?.id
                : params.data?.id;
            if (!id) {
            ...
            queryClient.setQueryData([resource, 'getOne', { id: String(id), meta: params.meta }], (record) => ({ ...record, ...clonedData }), { updatedAt });
            return clonedData;
        },
        ...
        getMutateWithMiddlewares: mutationFn => {
            if (getMutateWithMiddlewares) {
                ...
                const mutateWithMiddlewares = getMutateWithMiddlewares(dataProviderCreate.bind(dataProvider));
                return args => {
    ...
    const create = (0, util_1.useEvent)((callTimeResource = resource, callTimeParams = {}, callTimeOptions = {}) => {
        return mutate({
